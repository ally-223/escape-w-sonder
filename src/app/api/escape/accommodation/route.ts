...
import { fallbackAccommodations } from "@/lib/escape/mockAccommodation";
import { nextWeekend } from "@/lib/escape/dates";
import type { Accommodation, DestinationId } from "@/lib/escape/types";
...
function mapStay22Result(result: Stay22Result): Accommodation | null {
  ...
  return {
    ...
    pricePerNightTotal: Math.round(supplier.price.total / NIGHTS),
...
export async function GET(request: NextRequest) {
  ...
  const destinationId = searchParams.get("destination") as DestinationId | null;
  const groupSize = Number(searchParams.get("adults") ?? "2") || 2;
  ...
  if (!apiKey) {
    return NextResponse.json({
      accommodations: fallbackAccommodations(destinationId),
      source: "fallback",
    });
  }
  ...
  try {
    ...
    url.searchParams.set("address", `${destination.name}, ${destination.province}, Canada`);
